...
# TensorBoard writer
writer = SummaryWriter(log_dir='runs/experiment_tune')
...
# Argument parsing
parser = argparse.ArgumentParser(description='Fine-tune with Outlier Exposure (OE)')
...
parser.add_argument('--save', '-s', type=str, default='./snapshots/oe_tune')
parser.add_argument('--load', '-l', type=str, default='./snapshots/baseline_0831')
...
# === Paths ===
VIDEO_DIR_IN = '/your/path/to/ucf101/videos'
VIDEO_DIR_OUT = '/your/path/to/kinetics/videos'
LABEL_FILE_TRAIN_IN = '/your/path/to/ucf101_train_labels.txt'
LABEL_FILE_TRAIN_OUT = '/your/path/to/kinetics_train_labels.txt'
LABEL_FILE_TEST = '/your/path/to/ucf101_val_labels.txt'
NUM_CLASSES = 93
...
train_dataset_in = VideoDataset(VIDEO_DIR_IN, LABEL_FILE_TRAIN_IN, clip_len=8, frame_interval=8, num_clips=1, transform=train_transform)
train_loader_in = DataLoader(train_dataset_in, batch_size=args.batch_size, shuffle=True, num_workers=args.prefetch)

train_dataset_out = VideoDataset(VIDEO_DIR_OUT, LABEL_FILE_TRAIN_OUT, clip_len=8, frame_interval=8, num_clips=1, transform=train_transform)
...
def train():
    ...
    for in_set, out_set in tqdm(zip(train_loader_in, train_loader_out), total=min(len(train_loader_in), len(train_loader_out))):
        ...
        # OE loss: in-distribution + uniform regularization
...
def test():
    net.eval()
    loss_total, correct_top1, correct_top5 = 0.0, 0, 0
    global global_step
...
log_file = os.path.join(args.save, 'oe_tune_training_results.csv')
with open(log_file, 'w') as f:
    f.write('epoch,time(s),train_loss,test_loss,test_error(%),top1,top5\n')
...
for epoch in range(start_epoch, args.epochs):
    ...
    if (epoch + 1) % save_interval == 0:
        ckpt_path = os.path.join(args.save, f'oe_tune_epoch_{epoch + 1}.pt')
        torch.save(net.state_dict(), ckpt_path)
        print(f'Checkpoint saved: {ckpt_path}')

    with open(log_file, 'a') as f:
        f.write('%03d,%05d,%0.6f,%0.5f,%0.2f,%0.4f,%0.4f\n' % (
            epoch + 1,
